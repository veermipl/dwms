...
<div style="font-weight: bold; font-size: 8pt; font-style: italic;">
    ...
    function tariff()
    {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['tariff'] = $this->finance_model->getTariff();
        $data['membership'] = $this->finance_model->getMembership();
        // $data['doctors'] = $this->doctor_model->getDoctor();
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('finance/tariff', $data);
        $this->load->view('home/footer'); // just the footer file
    ...
    {   
        ...
        if (empty($redirect)) {
            $redirect = 'finance/tariff';
        }   

        $id = $this->input->post('id');
        $doctor = $this->input->post('doctor_id');
        $membership = $this->input->post('membership_details_id');
        $amount = $this->input->post('amount');
        ...
        if (empty($amount)) {
         ...
         $data = array(
                'doctor_id' => $doctor,
                'membership_details_id' => $membership,
                'amount' => $amount,
            );
            ...
            if (!empty($id)) {
                echo "update";
                $this->session->set_flashdata('feedback', lang('updated'));
                $this->finance_model->updateTariff($id, $data);
            } else {
                $this->session->set_flashdata('feedback', lang('added'));
                $this->finance_model->insertTariff($data);
            }
    ...
    function editTariffByJason() {
        $id = $this->input->get('id');
        // echo $id; exit;
        $data['tariff'] = $this->finance_model->getTariffById($id);
        echo json_encode($data);
    }
    ...
    function deleteTariff() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
            $id = $this->input->get('id');
            $this->finance_model->deleteTariff($id);
            // $this->finance_model->deleteDepositByInvoiceId($id);
            $this->session->set_flashdata('feedback', lang('deleted'));
            redirect('finance/tariff');
        }
