...

{
...

    {
        ...
        if (!empty($this->input->get('weekday'))) {
            ...
            $the_session = array("weekday" => $radio1);
        ...
        $data['schedules'] = $this->schedule_model->getSchedule($radio1);
...

    {
        ...
        $data['schedules'] = $this->schedule_model->getScheduleByDoctor($doctor, $radio1);
...

    {
        ...
        if (!empty($id)) {
            ...
            $previous_time = $this->schedule_model->getScheduleByDoctorByWeekday($doctor, $weekday, $location, $s_time, $e_time);
        }
...

    {
        ...
        $this->schedule_model->deleteTimeSlotByDoctorByWeekday($doctor, $weekday, $location, $s_time, $e_time);
    ...
    {
        if ($id) {
            $data['locationData'] = $this->schedule_model->getLocationById($id);
    ...
    function storeLocation()
    {
        ...
        $this->form_validation->set_rules('loc_code', 'Location Code', 'trim');
        $this->form_validation->set_rules('loc_address', 'Location Address', 'trim');

        $id = htmlentities($this->input->post('loc_id'));

        if ($this->form_validation->run() === FALSE) {
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            $name = htmlentities($this->input->post('loc_name'));
            $code = htmlentities($this->input->post('loc_code'));
            $location_address = htmlentities($this->input->post('loc_address'));
            ...
            if ($id && !empty($id) && isset($id)) {
                $this->session->set_flashdata('feedback', lang('updated'));
                $this->schedule_model->updateLocation($id, $data);
            } else {
                $this->session->set_flashdata('feedback', lang('added'));
                $this->schedule_model->insertLocation($data);
            }
    ...
    function addLocation()
    {
        ...
        $location_address = htmlentities($this->input->post('location_address'));
    ...
    {
        $id = htmlentities($this->input->get('id'));

        $location = $this->schedule_model->getLocationById($id);
...

    {
        ...
        $this->schedule_model->deleteLocation($id);
