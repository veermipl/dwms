...

{
    ...
    function getBloodGroupById($id = null)
    ...
    function getDoctorInfo($searchTerm)
    {
        ...
        if (!empty($searchTerm)) {
            ...
            $this->db->from('users');
            $this->db->join('doctor', 'users.id = doctor.ion_user_id', 'right');
            // $this->db->where("doctor.name like '%" . $searchTerm . "%' ");
            // $this->db->or_where("doctor.id like '%" . $searchTerm . "%' ");
        ...
        if ($this->ion_auth->in_group(array('Doctor'))) {
            ...
            $this->db->from('users');
            $this->db->join('doctor', 'users.id = doctor.ion_user_id', 'right');
            $this->db->where('users.ion_user_id', $doctor_ion_id);
            $this->db->where('users.active', 1);
            $fetched_records = $this->db->get();
