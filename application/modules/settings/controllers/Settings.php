...
class Settings extends MX_Controller {
    ...
    public function update() {
        ...
        $maxdays_for_booking = $this->input->post('maxdays_for_booking');
        $mindays_for_booking = $this->input->post('mindays_for_booking');
        $default_service_time = $this->input->post('default_service_time');
        $unpaid_appointment = $this->input->post('unpaid_appointment');
        $allow_client = $this->input->post('allow_client');
        $send_sms_push = $this->input->post('send_sms_push');
        $first_tax_name = $this->input->post('first_tax_name');
        $first_tax_gst_rate = $this->input->post('first_tax_gst_rate');
        $second_tax_name = $this->input->post('second_tax_name');
        $second_tax_gst_rate = $this->input->post('second_tax_gst_rate');
        $invoice_footer_note = $this->input->post('invoice_footer_note');
       //echo '<pre>';print_r($_POST); die;
        if (!empty($email)) {
            ...
            $this->form_validation->set_rules('maxdays_for_booking', 'Max Day For Booking');
            $this->form_validation->set_rules('mindays_for_booking', 'Max Day For Booking');
            $this->form_validation->set_rules('default_service_time', 'Default Service Time');
            $this->form_validation->set_rules('unpaid_appointment', 'Unpaid Appointment');
            $this->form_validation->set_rules('allow_client', 'Allowed Client');
            $this->form_validation->set_rules('send_sms_push', 'Send SMS Push');
            $this->form_validation->set_rules('first_tax_name', 'First Tax Name');
            $this->form_validation->set_rules('first_tax_gst_rate', 'First Tax GST Rate');
            $this->form_validation->set_rules('second_tax_name', 'Second Tax Name');
            $this->form_validation->set_rules('second_tax_gst_rate', 'Second Tax GST Rate');
            $this->form_validation->set_rules('invoice_footer_note', 'Invoice Footer Note');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->upload->do_upload('img_url')) {
                    ...
                    $data = array(
                        ...
                        'logo' => $img_url,
                        'timezone' => $timzone,
                        'maxdays_for_booking' => $maxdays_for_booking,
                        'mindays_for_booking' => $mindays_for_booking,
                        'default_service_time' => $default_service_time,
                        'unpaid_appointment' => $unpaid_appointment,
                        'allow_client' => $allow_client,
                        'send_sms_push' => $send_sms_push,
                        'first_tax_name' => $first_tax_name,
                        'first_tax_gst_rate' => $first_tax_gst_rate,
                        'second_tax_name' => $second_tax_name,
                        'second_tax_gst_rate' => $second_tax_gst_rate,
