...
function login() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'))) {
              ...
              if(!empty($deviceTokken)){
               ...
               $message=  preg_replace("/\r|\n/", "", strip_tags($this->ion_auth->errors()));
                echo json_encode([ 'data'=>'{}','status'=>"false",'message'=> $message]);
            ...
            echo json_encode(['data'=>'{}','status'=>"false",'message'=>$data['message']]);
   ...
   function check_user( ) {
	...
	function create_user() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $p_result=$this->patient_model->insertPatient($p_data);
            if ($p_result==true) {
                $this->db->where('ion_user_id',$userdata[0]->id);
            ...
            echo json_encode(['data'=>$result1[0],'status'=>'true','message'=>preg_replace("/\r|\n/", "", strip_tags($this->ion_auth->messages()))]);
        } else {
            //display the create user form
            //set the flash data error message if there is one
            $message= (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
...
function book_appointment() {
    ...
    if ($this->form_validation->run() == true) {
      ...
      //  $userdata =  $this->ion_auth->where('id',$userid)->users()->result();
        ...
        echo json_encode(['data'=>$deviceToken ,'status'=>'true','message'=>'Appointment Booked Successfully']);
    } else {
        //display the create user form
        //set the flash data error message if there is one
        $message= (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
...
function  ApproveOrReject(){
   ...
   $appointment_id= $_POST['appointment_id'];
    ...
    $data_result=array();
      ...
      $data_result['patientname']=$appointment_details[0]->patientname;
      $data_result['BookingDate']=date('d/m/Y', $appointment_details[0]->date);
...
public function otpverify() {
    ...
    if($get_otp!="" && $get_otp != null) {
      ...
      if($get_otp == $result[0]->otp) {
            ...
            echo json_encode(['data'=>'{}','status'=>"true",'message'=>"OTP Verify successfully"]);
             
            }
         
         else{
            echo json_encode(['data'=>'{}','status'=>"false",'message'=>"New OTP & Confirm OTP is not matching"]);  
...
public function resetPassword() {
        ...
        if($new_password == $conf_password) {
                if($this->Api_model->updatePassword($new_password, $userid)){
                     echo json_encode(['data'=>'{}','status'=>"true",'message'=>"Password updated successfully."]);
                }
                else{
                   
                     echo json_encode(['data'=>'{}','status'=>"False",'message'=>"Failed to update password."]);
                }
            }
        
        else{
         
             echo json_encode(['data'=>'{}','status'=>"False",'message'=>"New password & Confirm password is not matching."]);
        }
