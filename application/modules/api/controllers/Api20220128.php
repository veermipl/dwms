...
class Api extends MX_Controller {
    ...
    function base64_to_jpeg() {
        ...
        echo $output_file_with_extension;
    }
    ...
    function getPatientByDoctorIdbyjason() {
        ...
        foreach ($appointments as $appointment) {
            ...
            if (!empty($patient_exists)) {
                ...
                $patient['name'] = $patient_exists->name;
                $patient['booking_date'] = $appointment->date;
                $patient['booking_time'] = $appointment->s_time;
        ...
        // } else {
        //     echo json_encode(['data'=>'{}','status'=>'false','message'=>'unsuccessfull']);
        // }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'))) {
               ...
               $this->db->where('ion_user_id',$userdata[0]->id);
                ...
                $doctor_data = $query->result();
                
               // echo json_encode([200,$userdata]);
                echo json_encode(['data'=>$userdata[0]->id,'status'=>"true",'message'=>"success"]);
   ...
   // check user 
    function check_user( ) {
        $this->db->where('email',$_GET['email']);
            ...
            $num = $query->num_rows();  
            if($num >=1){
                echo json_encode(['data'=>$_GET['email'],'status'=>"true",'message'=>"User Found"]);

            } else {
                echo json_encode(['data'=>$_GET['email'],'status'=>"false",'message'=>"No User Found"]);
...
function googlesigningdetails() {
    ...
    if ($this->form_validation->run() == true) {
     ...
     echo json_encode(['data'=>['status'=>'no'],'status'=>'false','message'=>preg_replace("/\r|\n/", "", strip_tags($message))]);
    }
}
